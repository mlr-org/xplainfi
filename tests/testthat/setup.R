...
# CRAN policy: at most 2 cores in tests. data.table (OpenMP) and ranger (own std::thread pool, also via arf)
# are capped here; an OpenMP BLAS cannot be capped from inside R (libgomp reads OMP_* once at load), CRAN
# sets OMP_THREAD_LIMIT externally for that. Restored in teardown.
