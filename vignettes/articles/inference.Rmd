...
the standard errors are the running variance across permutations for the permutation estimator ([Covert et al., 2020](https://arxiv.org/abs/2004.00668)), the closed-form covariance of [Covert & Lee (2021)](https://proceedings.mlr.press/v130/covert21a.html), Eqs. 12-13, for the default kernel variant, and their batch-means estimate (Section 4.3) for `kernel_variant = "original"`, so coverage is asymptotic in the number of sampled coalitions.
The package's test suite checks the standard errors against the actual run-to-run variability of the estimates, but no systematic coverage study exists yet, so treat the intervals as approximate.
...
the default `"original"` follows the estimator Covert & Lee (2021) recommend for practical use (their Section 4.1), while `"unbiased"` matches the estimator implemented in the reference Python [sage](https://github.com/iancovert/sage) package and is kept as a parity check.
The test suite checks both variants' standard errors against replicate variability; at equal coalition budgets the unbiased variant's point estimates are substantially noisier, which its wider intervals reflect.
