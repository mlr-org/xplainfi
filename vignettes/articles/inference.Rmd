...
the default `"unbiased"` matches the estimator implemented in the reference Python [sage](https://github.com/iancovert/sage) package, while `"original"` follows the estimator Covert & Lee (2021) recommend for practical use (their Section 4.1).
Both provide calibrated intervals, but at equal coalition budgets the original variant's point estimates are substantially less noisy, which its narrower intervals reflect.
...
```{r sage-montecarlo-original}
#| fig.alt: "Point-and-whisker plot with features on y-axis and SAGE values on x-axis. Two colored series for the unbiased and original kernel variants; the original variant shows markedly narrower horizontal error bars at the same coalition budget."
set.seed(1)
sage_original = MarginalSAGE$new(
	task = task_sage,
	learner = learner_sage,
	measure = measure,
	resampling = resampling_sage,
...
set.seed(2)
sage_original$compute()

sage_cis = rbindlist(list(
	sage$importance(ci_method = "montecarlo")[, variant := "unbiased"],
	sage_original$importance(ci_method = "montecarlo")[, variant := "original"]
))
...
ggplot(sage_cis, aes(y = feature, color = variant)) +
	...
	labs(
		...
		subtitle = "Same coalition budget; the original variant is markedly less noisy",
