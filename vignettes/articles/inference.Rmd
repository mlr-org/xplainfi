...
This applies to all `ci_method`s that produce p-values (`"raw"`, `"nadeau_bengio"`, `"cpi"`, `"lei"`, `"montecarlo"`).
...
they answer "how precisely has coalition sampling pinned down this model's SAGE values", not "how important is this feature under the data-generating process".
For learner-level questions, the resampling-based methods above are the relevant (if imperfect) tools.
...
As with the other parametric methods, `alternative` and `p_adjust` are supported, and `"bonferroni"` also adjusts the confidence intervals.
...
Both provide calibrated intervals, but at equal coalition budgets the unbiased variant's point estimates are substantially noisier, which its wider intervals honestly reflect:
