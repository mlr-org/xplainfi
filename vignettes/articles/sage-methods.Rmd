...
**The permutation estimator** (`estimator = "permutation"`, the default) is what is referred to as the "permutation estimator" in other implementations.
...
Any combination is valid, e.g. conditional kernel SAGE reuses the exact same conditional value function as the permutation estimator and only changes how coalitions are sampled.
...
Because the estimator axis is orthogonal to how out-of-coalition features are marginalized, `estimator = "kernel"` works with both `MarginalSAGE` and `ConditionalSAGE`.
...
For this ten-feature task the two estimators are comparably cheap, and the permutation estimator additionally gives you convergence diagnostics, so there is little reason to switch.
