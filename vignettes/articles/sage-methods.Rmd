...
- **Permutation** evaluates `n_permutations * p + 1` coalitions.
  ...
  In return it provides standard-error-based convergence tracking and early stopping (shown above).
- **Kernel** evaluates `2 * n_coalitions + 2` coalitions (each sampled coalition is paired with its complement for variance reduction), independent of $p$.
  ...
  The current implementation does not yet expose standard errors or early stopping for this estimator.
...
- For a handful of features, prefer **permutation**: it is simple, cheap, and provides standard errors and early stopping.
...
It carries no coalition-sampling error, so for `MarginalSAGE` it yields the exact SAGE values that the permutation and kernel estimators approximate -- which makes it a convenient ground-truth reference when validating or comparing estimators.
