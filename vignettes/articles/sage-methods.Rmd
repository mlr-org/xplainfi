...
`xplainfi` provides two **estimators** for approximating SAGE values, selected with the `estimator` argument.
...
The two estimators approximate the same SAGE values but scale differently with the number of features; the [Choosing an estimator](#choosing-an-estimator) section below discusses when to prefer each.
