...
See `vignette("perturbation-importance")` for detailed comparisons and `vignette("feature-samplers")` for more details on implemented samplers.
...
- **SAGE**: Currently does not support parallelization and will always run sequentially in terms of the permutations evaluated. The learner or resampling procedure used for the initial model fits can be parallelized though.
- **Performance**: Parallelization is most beneficial with multiple features, large datasets, or expensive learners
- Use either `future::plan()` or `mirai::daemons()` - both work with the same code, similar to mlr3
