...
- `PFI`, `CFI`, and `RFI` no longer report integer positions instead of
  feature (or group) names in the `feature` column of `$scores()`,
  `$importance()`, and `$obs_loss()` when a non-sequential
