# Benchmark ConditionalSAGE + ConditionalARFSampler before/after the
# samples_per_row refactor.
#
# Compares the current branch (where ConditionalSAGE passes
# `samples_per_row = n_samples` to the sampler on unique test rows) against
# the pre-refactor commit (where test rows were replicated `n_samples`
...
result = cross::bench_branches(
	branches = c(REF_PRE, "HEAD"),
	{
		...
		bench::mark(sage$compute(), iterations = 3L, check = FALSE)
	}
)
