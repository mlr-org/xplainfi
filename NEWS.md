...
- `MarginalSAGE` and `ConditionalSAGE` gain an `estimator` argument selecting the Shapley-value estimator (#70).
  ...
  - If unset, `n_permutations` defaults to `10L` (reduced on small feature sets so it stays at or below half the cost of exact enumeration) and `n_coalitions` to `100 * n_features` draws (capped at `4096L`), sized for the default kernel variant.
...
- `SAGE` kernel estimator gains a `kernel_variant` argument (#70, #71).
  - `"unbiased"` (the default) is Covert & Lee's Eq. 9, the estimator implemented by the reference Python `sage` package, so results are directly comparable.
  - `"original"` is their Eq. 7, recommended by the paper for practical use and typically reaching a given accuracy at an order of magnitude smaller budgets in our setting.
