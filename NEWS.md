...
- `CFI` gains an `estimator` argument. `"sampling"` (the default) is the previous behavior. `"weighting"` permutes features marginally as in `PFI` and weights each perturbed observation by the ARF-estimated density ratio `p(x_j' | x_-j) / p(x_j')`, an importance sampling estimator of the same conditional target that only needs density evaluations (`arf::lik()`) instead of conditional draws (`arf::forge()`) and shares its perturbed data with `PFI`. Runtime is comparable to `"sampling"`. Requires a `ConditionalARFSampler` and a measure with the `"weights"` property; `ci_method = "cpi"` is available only with `normalize = "per_observation"` and has not been validated for it. `$scores()` reports the effective sample size of the weights as `ess`.
- `PerturbationImportance` methods (`PFI`, `CFI`, `RFI`) gain a `weight_fun` argument, a `function(data, feature)` returning non-negative observation weights for the perturbed data. Weights are passed to the measure via `Prediction$weights`, so the measure must have the `"weights"` property. `$obs_loss()` gains a `weight` column (scaled to mean 1 within its normalization group, with `obs_importance` based on `weight * loss_post`) and `$scores()` an `ess` column when weights are used.
- `PerturbationImportance` methods also gain a `normalize` argument for how those weights are normalized: `"global"` (default) across observations within each repeat, or `"per_observation"` across the repeats of each observation, which yields per-observation conditional expectations and unlocks `ci_method = "cpi"` (unvalidated). `"per_observation"` requires a measure that aggregates as the mean of observation-wise losses (e.g. `regr.mse`, `classif.logloss`; not `regr.rmse`); its `ess` is the mean per-observation effective sample size, at most `n_repeats`.
- `PerturbationImportance$reweight(weight_fun, normalize)` re-scores the predictions stored by `$compute()` with a different weight function (or none), without perturbing or predicting again. With `weights_arf()` this turns one `PFI` prediction pass into PFI, CFI, and any RFI conditioning set. `$reset()` also clears the stored perturbed values.
- `RFI` gains the same `estimator` argument as `CFI`. With `"weighting"`, the conditioning set only changes the weights `p(x_j' | x_S) / p(x_j')`, so `$compute(conditioning_set = ...)` re-uses marginal permutation for any `S`; `character(0)` reproduces `PFI`.
- `weights_arf()` creates the ARF-based importance sampling weight function used by `CFI` and `RFI` with `estimator = "weighting"` from a fitted `ConditionalARFSampler`, for use with `weight_fun`. Its `conditioning_set` argument selects the RFI conditioning set (`NULL` for CFI). For feature groups the proposal density is the product of the per-feature marginals, matching the column-wise permutation of `MarginalPermutationSampler`.
- `MarginalSAGE` and `ConditionalSAGE` gain an `estimator` argument selecting the Shapley-value estimator (#70).
  ...
  - `"kernel"` (`MarginalSAGE` only) is the regression-based estimator of Covert & Lee (2021) as implemented by the Python `sage` package: unbiased KernelSHAP with paired coalition sampling on the stochastic SAGE game, i.e. every coalition draw is evaluated on a single test observation via the measure's observation-wise loss. It is controlled by the new `n_coalitions` budget argument (default `2048L`) and requires a measure with the `"obs_loss"` property. Standard errors and early stopping are planned.
  - `"exact"` enumerates all coalitions on small feature sets (capped by the new `max_features` argument) and computes SAGE values without coalition-sampling error, useful as a ground-truth reference for the sampling estimators. It takes no budget; setting `n_permutations` or `n_coalitions` with it is an error, and `$budget` reports `converged = NA`.
  - Setting the budget argument of a different estimator is an error; setting a convergence argument for an estimator that ignores it is a warning.
...
- `CFI$new()` now forwards its `relation` argument; it was previously ignored and `"difference"` always used.
- `RFI$compute(conditioning_set = ...)` no longer errors with "cannot change value of locked binding for 'scores'"; overriding the conditioning set at compute time previously never worked.
